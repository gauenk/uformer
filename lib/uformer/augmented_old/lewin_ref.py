...
class LeWinTransformerBlockRefactored(nn.Module):
    ...
    def forward(self, x, H, W, mask=None, flows=None, state=None):
        ...
        assert self.cross_modulator is None
        if self.cross_modulator is not None:
            x_rs = x.contiguous().view(B*T,C,H*W).transpose(1,2).contiguous()
            shortcut = x_rs
        ...
        # -- norm layer --
        x = x.view(B*T,C,H*W).contiguous().transpose(1,2).contiguous()
        x = self.norm1(x)
        x = x.transpose(1,2).contiguous().view(B, T, C, H, W)
        ...
        # -- view for ffn --
        x = x.view(B*T,C,H*W).contiguous().transpose(1,2).contiguous()
        shortcut = shortcut.view(B*T,C,H*W).contiguous().transpose(1,2).contiguous()
        ...
        x = x.transpose(1,2).contiguous().view(B,T,C,H,W).contiguous()
        return x
